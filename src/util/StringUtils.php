...
final class StringUtils
{
    ...
    public static function ucwords(string $str, string $joinBy = '-', string... $delimiters): string
    {
        if (empty($delimiters) || !ArrayUtils::isStringArray($delimiters)) {
            ...
            $delimiters = array_filter($delimiters, function ($it) {
                return strlen($it) === 1;
            });
    ...
    public static function slice(string $str, int $start, int $end = PHP_INT_MIN): string
    {
        ...
        if ($end < $start) {
            $nums = array_filter($posRange, function ($it) use ($start) {
                return $it >= $start;
            });
        } else {
            $nums = array_filter($posRange, function ($it) use ($start, $end) {
        ...
        $parts = array_map(function ($it) use ($chars) {
            return $chars[$it];
        }, $nums);
